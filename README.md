...
- **Backend:** Firebase Authentication (email/password and Google) and Cloud Firestore storing `profiles`, `conversations`, per-conversation `messages`, ephemeral `secretPackets`, and reusable `invites` records.
...
- A Firebase project with Authentication and Cloud Firestore enabled (turn on both **Email/Password** and **Google** providers).
...
1. Visit `login.html`, create an account from the **Sign Up** tab (or use Google sign-in), and ensure you are redirected to `index.html` once authenticated.
2. Use the **Start a conversation** form to open a general chat with an existing user and confirm messages persist across refreshes.
3. Start a **Secret** chat and verify that messages stay on the device (reload the page to see them restored locally) while deliveries to the other participant vanish after reading.
4. Enter a non-member email to generate an invite link; register with that link in a separate browser and confirm the conversation is created automatically.
5. Toggle light/dark/system themes to ensure color contrast remains high, and simulate offline mode to observe graceful error handling.
